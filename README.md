...
- `permissoes_usuario`: tabela para permissões granulares no painel administrativo.

### Autenticação
- `public/register.php`: cadastro com hash seguro (`password_hash`).
- `public/login.php`: login com `password_verify` (aceita usuário ou e-mail).
...
### Painel admin
- `admin/dashboard.php`: dashboard moderno, usuários, permissões por usuário, pastas, arquivos, categorias e logs.
- Aprovação de conteúdo, bloqueio/reativação de usuários, remoção de pastas e controle de permissões granulares.
...
## 9) Versionamento sem binários

- O repositório não versiona arquivos binários de upload nem pacotes `.zip`.
- As pastas `uploads/files` e `uploads/covers` são mantidas com `.gitkeep`.
- Para gerar pacote local, use as instruções em `DOWNLOAD.md`.
